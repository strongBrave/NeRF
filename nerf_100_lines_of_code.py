...
def render_rays(nerf_model, ray_origins, ray_directions, hn=0, hf=0.5, nb_bins=192):
    ...
    t = torch.linspace(hn, hf, nb_bins, device=device).expand(ray_origins.shape[0], nb_bins) # [batch_size, n_bins]
    # Perturb sampling along each ray.
    mid = (t[:, :-1] + t[:, 1:]) / 2. # [b, nb - 1]
    lower = torch.cat((t[:, :1], mid), -1) # [b, nb]
    upper = torch.cat((mid, t[:, -1:]), -1) # [b ,nb]
    u = torch.rand(t.shape, device=device) # [b, nb]
    ...
    sigma = sigma.reshape(x.shape[:-1]) # [b, nb]

    alpha = 1 - torch.exp(-sigma * delta)  # [batch_size, nb_bins]
    # weights = compute_accumulated_transmittance(1 - alpha).unsqueeze(2) * alpha.unsqueeze(2)
    alpha_shifted = torch.cat([torch.ones(alpha.shape[0], 1), 1 - alpha+1e-10], dim=-1)
    weights = alpha * torch.cumprod(alpha_shifted, dim=1)[:, :-1] # [b, nb]
